"""Core benchmark loop: atlas × metric × algo × band."""
...
from pipeline_mne_bids.bench.reporter import dump_json, write_summary
...
def run_bench(
    ...
    *,
    ...
    """Esegue la matrice di benchmark completa (700 run: 7×4×5×5).
    ...
    epochs, inv_op, y, sfreq = _load_epochs_and_inv(sub_eeg, subject, sfreq_target, n_epochs_max)
            ...
            for metric in METRICS:
                ...
                for algorithm in ALGORITHMS:
                    ...
                    if run_id % dump_every == 0:
                        dump_json(out_json, meta, runs)

    meta["ts_end"] = datetime.now(_TZ).isoformat()
    dump_json(out_json, meta, runs)
    write_summary(out_summary, meta, runs)
